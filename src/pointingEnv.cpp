         ...
         char **argv)
{
    ...
    std::cerr << "Fg: " << Fg << '\n';
    realT beta_p = 5.0; // Bad Zernike, to-do: what is beta_p of LLOWFS?
    ...
    for (realT f = fsMin; f <= fsMax; f *= dfs)
        fs.push_back(f);

    logfs.resize(fs.size());
    for (size_t n = 0; n < fs.size(); ++n)
        logfs[n] = log10(fs[fs.size() - 1 - n]);
    ...
    for (realT s = sigMin; s <= sigMax; s *= dsig)
        sig_nm.push_back(s);
    ...
    for (realT a = alphaMin; a < alphaMid; a += dalphaMid)
        alpha.push_back(a);
    for (realT a = alphaMid; a < alphaMax * dalpha * 0.99; a *= dalpha)
        alpha.push_back(a);
...
#pragma omp parallel for
    ...
    if (mx::mxlib_comp_repo_modified())
        fout << " (modified)";
    fout << "\n";
    ...
    if (POINTINGENV_GIT_REPO_MODIFIED)
        fout << " (modified)";
    fout << "\n";
